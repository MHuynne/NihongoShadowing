...
router = APIRouter(prefix="/roleplay", tags=["Roleplay Contextual"])


@router.post("/scenarios", response_model=schemas_roleplay.RoleplayScenarioResp)
def create_scenario(
    scenario_req: schemas_roleplay.RoleplayScenarioBase,
...
def get_scenarios(db: Session = Depends(get_db)):
    return crud_roleplay.get_scenarios(db)


@router.get("/scenarios/{scenario_id}", response_model=schemas_roleplay.RoleplayScenarioResp)
def get_scenario(scenario_id: int, db: Session = Depends(get_db)):
    scenario = crud_roleplay.get_scenario(db, scenario_id)
    if scenario is None:
        raise HTTPException(status_code=404, detail="Scenario not found")
    return scenario


@router.put("/scenarios/{scenario_id}", response_model=schemas_roleplay.RoleplayScenarioResp)
def update_scenario(
    scenario_id: int,
    scenario_req: schemas_roleplay.RoleplayScenarioBase,
    db: Session = Depends(get_db),
):
    scenario = crud_roleplay.update_scenario(db, scenario_id, scenario_req)
    if scenario is None:
        raise HTTPException(status_code=404, detail="Scenario not found")
    return scenario
...
def delete_scenario(scenario_id: int, db: Session = Depends(get_db)):
    scenario = crud_roleplay.delete_scenario(db, scenario_id)
    if scenario is None:
        raise HTTPException(status_code=404, detail="Scenario not found")
    return None
...
def create_session(
    session_req: schemas_roleplay.SessionCreateReq,
    db: Session = Depends(get_db),
):
    return crud_roleplay.create_session(db, session_req)
...
def update_mode(
    session_id: int,
    mode_req: schemas_roleplay.SessionModeUpdateReq,
    db: Session = Depends(get_db),
):
...
async def chat_with_ai(
    chat_req: schemas_roleplay.ChatRequestReq,
    db: Session = Depends(get_db),
):
    ...
    grammar_dict = (
        ai_response.grammar_correction.model_dump()
        if ai_response.grammar_correction
        else None
    )
