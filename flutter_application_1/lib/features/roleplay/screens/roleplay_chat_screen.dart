...
class _RoleplayChatScreenState extends State<RoleplayChatScreen> {
  ...
  String _textBeforeCurrentSegment = ''; // Lưu trữ văn bản cũ trước khi bắt đầu lượt nghe mới
    ...
    try {
      _isSpeechInitialized = await _speech.initialize(
        ...
        onError: (error) {
          ...
          if (error.errorMsg.contains('no_match') || error.errorMsg.contains('timeout')) {
  ...
  Future<void> _initializeChat() async {
    ...
    try {
      final scenarioId = await _apiService.getOrCreateScenario(widget.title, widget.description);
      final sessionId = await _apiService.createSession(scenarioId, widget.mode);
      
      setState(() {
        _sessionId = sessionId;
        _isLoading = false;
        _addMessage('Sensei: Xin chào! Bối cảnh đã sẵn sàng. Mời bạn bắt đầu hội thoại.', false);
      });
    } catch (e) {
      setState(() => _isLoading = false);
      _addMessage('Lỗi kết nối Server: Hãy chắc chắn bạn đã chạy Backend Python!', false);
    }
  }
  ...
  Future<void> _handleSendText(String text) async {
    ...
    try {
      final response = await _apiService.chatWithAI(_sessionId!, text);
  ...
  Future<void> _startListening() async {
    ...
    try {
      if (_isSpeechInitialized) {
        ...
        setState(() {
          _isRecording = true;
          _textBeforeCurrentSegment = _textController.text; // Giữ lại nội dung cũ
          // Nếu ô chat đã có chữ thì thêm dấu cách để nối từ cho đẹp
          if (_textBeforeCurrentSegment.isNotEmpty && !_textBeforeCurrentSegment.endsWith(' ')) {
            _textBeforeCurrentSegment += ' ';
          }
          _currentVoiceText = '';
        });
        ...
        */
        ...
        await _speech.listen(
          onResult: (result) {
            if (result.recognizedWords.isNotEmpty) {
              setState(() {
                ...
                _textController.text = _textBeforeCurrentSegment + _currentVoiceText;
          ...
          onDevice: false, // Tắt chế độ offline để tránh lỗi language_unavailable
          cancelOnError: false,
          partialResults: true,
        );
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Lỗi: Không thể khởi động Mic. Hãy kiểm tra quyền truy cập!')),
          );
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const Text(
                      'AI Sensei Online',
                      style: TextStyle(fontSize: 12, color: AppColors.slate500, fontWeight: FontWeight.normal),
                    ),
                  ],
                ),
              ],
      ...
      body: Column(
        children: [
          if (_isLoading) const LinearProgressIndicator(minHeight: 2, color: AppColors.primary, backgroundColor: Colors.transparent),
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.auto_fix_high, size: 16, color: AppColors.primary.withOpacity(0.7)),
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        suffixIcon: IconButton(
                          ...
                          onPressed: () => _handleSendText(_textController.text),
                        ),
                      ),
                      onSubmitted: (val) => _handleSendText(val),
                    ),
                  ),
                ),
              ],
